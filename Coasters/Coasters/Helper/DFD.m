...
+ (void)performBlockOnce:(void (^)())block
{
...
+ (NSDate *)getDateFromString:(NSString *)strTime
{
    ...
    return [DFD clearTimeZone:[DFD getDateFromArr:@[@(hour),@(minute),@0,@0]]];
}
...
+ (UserInfo *)sharedManager
{
    @synchronized(self)
    {
        ...
        if (access) userInfo = [UserInfo findFirstByAttribute:@"access"
                                                    withValue:access
                                                    inContext:DBefaultContext];
        if (!userInfo && access)
...
//    userInfo.user_nick_name = @"CupcareUser";
//    userInfo.user_gender = @(NO);
//    userInfo.user_birthday = [DFD toDateByString:@"19900101"];
//    userInfo.unit = @(YES);
//    userInfo.user_drink_target = @(2000);
//    userInfo.swithLight = @(YES);
//    userInfo.swithSound = @(YES);
//    userInfo.swithNoDisturb = @(NO);
//    userInfo.noDisturbStart = @20;
//    userInfo.noDisturbEnd = @7;
...
+ (BOOL)isToday:(NSDate *)date
{
    if (!self) return NO;
    NSDate *now   = DNow;
    int year      = [DFD getFromDate:now type:1];
    int month     = [DFD getFromDate:now type:2];
    int day       = [DFD getFromDate:now type:3];

    int yearThis  = [DFD getFromDate:date type:1];
    int monthThis = [DFD getFromDate:date type:2];
    int dayThis   = [DFD getFromDate:date type:3];
...
{
    int hour   = [DFD getFromDate:date type:4];
    int minute = [DFD getFromDate:date type:5];
