...
- (void)initialize;
{
    ...
    _scaleDivisionColor = DRed;//RGB(68, 84, 105);
    _scaleSubDivisionColor = DLightGray;//RGB(217, 217, 217);
...
- (void)drawFace:(CGContextRef)context
{
    switch (_innerBackgroundStyle)
    {
        case WMGaugeViewInnerBackgroundStyleGradient:
        {
            ...
            CGGradientRef gradient = CGGradientCreateWithColors(baseSpace, (CFArrayRef)@[iCGRGB(255, 255, 255), iCGRGB(255, 255, 255), iCGRGB(255, 255, 255)], (const CGFloat[]){0.35, 0.96, 0.99});
        ...
        case WMGaugeViewInnerBackgroundStyleFlat:
        {
            ...
            CGContextSetFillColorWithColor(context, DClear.CGColor);
...
- (void)drawScale:(CGContextRef)context
{
    ...
    for (int i = 0; i < totalTicks; i++)
    {
        ...
        if ((fabsf(mod - 0) < 0.000001) || (fabsf(mod - div) < 0.000001))
        {
            ...
            CGContextSetStrokeColorWithColor(context, DWhite.CGColor);
            ...
            NSDictionary* stringAttrs = @{ NSFontAttributeName : font, NSForegroundColorAttributeName : DWhite };
...
- (void)drawNeedleScrew:(CGContextRef)context
{
    switch (_needleScrewStyle)
    {
        case WMGaugeViewNeedleScrewStyleGradient:
        {
            // Screw
            CGColorSpaceRef baseSpace = CGColorSpaceCreateDeviceRGB();
            CGGradientRef gradient = CGGradientCreateWithColors(baseSpace, (CFArrayRef)@[iCGRGB(171, 171, 171), iCGRGB(255, 255, 255), iCGRGB(255, 255, 255)], (const CGFloat[]){0.05, 0.9, 1.00});   // 中间圆球的颜色
            CGColorSpaceRelease(baseSpace), baseSpace = NULL;
            ...
            // Border
            CGContextSetLineWidth(context, 0.005);
            CGContextSetStrokeColorWithColor(context, CGRGBA(81, 84, 89, 100)); // 中间圆球   border 的颜色
            CGContextAddEllipseInRect(context, CGRectMake(center.x - _needleScrewRadius, center.y - _needleScrewRadius, _needleScrewRadius * 2.0, _needleScrewRadius * 2.0));
            CGContextStrokePath(context);
        }
        break;
        ...
        {
            CGContextAddEllipseInRect(context, CGRectMake(center.x - _needleScrewRadius, center.y - _needleScrewRadius, _needleScrewRadius * 2.0, _needleScrewRadius * 2.0));
            CGContextSetFillColorWithColor(context, DWhite.CGColor);
            CGContextFillPath(context);
        }
        break;
...
- (void)computeCurrentValue
{
    ...
    {
        NSTimeInterval time = ([DNow timeIntervalSince1970] - needleLastMoved);
        ...
        {
            needleLastMoved =  [DNow timeIntervalSince1970];
        }
